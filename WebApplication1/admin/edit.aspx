...
<asp:Content ID="Content4" ContentPlaceHolderID="Content" runat="server">
    ...
    <asp:FormView ID="FormView1" runat="server" DataKeyNames="ProductId" DataSourceID="SqlDataSource1" DefaultMode="Edit" BackColor="#FFFFCC" BorderColor="Black" BorderStyle="Ridge" BorderWidth="1px" HeaderText="Edit Product">
        <EditItemTemplate>
            ProductId:
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:db_1626501_asg_co5027ConnectionString %>" DeleteCommand="DELETE FROM [tblProduct] WHERE [ProductId] = @original_ProductId AND (([ProductName] = @original_ProductName) OR ([ProductName] IS NULL AND @original_ProductName IS NULL)) AND (([ProductDetails] = @original_ProductDetails) OR ([ProductDetails] IS NULL AND @original_ProductDetails IS NULL)) AND (([ProductQuantity] = @original_ProductQuantity) OR ([ProductQuantity] IS NULL AND @original_ProductQuantity IS NULL)) AND (([ProductPrice] = @original_ProductPrice) OR ([ProductPrice] IS NULL AND @original_ProductPrice IS NULL)) AND (([ProductLinkImage] = @original_ProductLinkImage) OR ([ProductLinkImage] IS NULL AND @original_ProductLinkImage IS NULL))" InsertCommand="INSERT INTO [tblProduct] ([ProductId], [ProductName], [ProductDetails], [ProductQuantity], [ProductPrice], [ProductLinkImage]) VALUES (@ProductId, @ProductName, @ProductDetails, @ProductQuantity, @ProductPrice, @ProductLinkImage)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT * FROM [tblProduct] WHERE ([ProductId] = @ProductId)" UpdateCommand="UPDATE [tblProduct] SET [ProductName] = @ProductName, [ProductDetails] = @ProductDetails, [ProductQuantity] = @ProductQuantity, [ProductPrice] = @ProductPrice, [ProductLinkImage] = @ProductLinkImage WHERE [ProductId] = @original_ProductId AND (([ProductName] = @original_ProductName) OR ([ProductName] IS NULL AND @original_ProductName IS NULL)) AND (([ProductDetails] = @original_ProductDetails) OR ([ProductDetails] IS NULL AND @original_ProductDetails IS NULL)) AND (([ProductQuantity] = @original_ProductQuantity) OR ([ProductQuantity] IS NULL AND @original_ProductQuantity IS NULL)) AND (([ProductPrice] = @original_ProductPrice) OR ([ProductPrice] IS NULL AND @original_ProductPrice IS NULL)) AND (([ProductLinkImage] = @original_ProductLinkImage) OR ([ProductLinkImage] IS NULL AND @original_ProductLinkImage IS NULL))">
            <DeleteParameters>
        ...
        <asp:Button ID="Btnbackedit" runat="server" Text="Back" OnClick="Btnbackedit_Click" />
    </form>
</asp:Content>
